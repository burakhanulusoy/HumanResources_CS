...
namespace HumanResources.Business.Validators.DiciplineValidators
{
    public class UpdateDiciplineValidator : AbstractValidator<UpdateDiciplineDto>
    {
        ...
        {
            RuleFor(x => x.Id)
                .GreaterThan(0).WithMessage("Geçerli bir kayýt ID'si gereklidir.");
            ...
            RuleFor(x => x.TanikAdSoyad)
                .MaximumLength(100).WithMessage("Tanýk 1 adý en fazla 100 karakter olabilir.");

            RuleFor(x => x.TanikAdSoyad2)
                .MaximumLength(100).WithMessage("Tanýk 2 adý en fazla 100 karakter olabilir.");
        }
    }
}